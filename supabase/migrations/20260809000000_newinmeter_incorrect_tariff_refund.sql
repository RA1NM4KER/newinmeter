-- Support "Incorrect Tariff Refund" (and any other "... Refund") ledger rows as
-- a first-class credit that reduces net spend, distinct from wallet top-ups.
--
-- The parser now stores these rows with the refund's own description as the
-- charge_label and a NEGATIVE cost. Two changes are needed here:
--   1. Classify them under a new charge_kind = 'refund' so they never count as
--      energy/water/fixed spend, top-ups, or usage.
--   2. Fold 'refund' into total_spend. Because the cost is negative, this
--      reduces net spend without touching any usage (kWh/kL) totals.
--
-- Historical refunds captured before this change were mislabelled "Top Up" by
-- the old parser and remain classified as 'topup' (their original description
-- is unrecoverable from stored data); only re-synced rows pick up the new type.
...
alter table public.energy_rows
  add column charge_kind text generated always as (
    case
      ...
      when lower(charge_label) like '%refund%' then 'refund'
      else 'fixed'
    end
  ) stored;
...
begin
  ...
  daily_aggregates as (
    select
      ...
      round(sum(case when charge_kind in ('energy', 'water', 'fixed', 'refund') then cost else 0 end)::numeric, 2) as total_spend,
