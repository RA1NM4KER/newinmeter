...
async function main() {
  const before = await adminSupabaseCount("/energy_rows?select=id&charge_kind=eq.energy&tariff_band=is.null&limit=1");
  ...
  while (true) {
    const rows = await adminSupabaseFetch<LedgerRow[]>(
      `/energy_rows?select=id,connection_id,charge_label,period_dt,tariff` +
        `&charge_kind=eq.energy&tariff_band=is.null&id=gt.${lastId}&order=id.asc&limit=${BATCH_SIZE}`
    );
    ...
    for (const [band, ids] of Array.from(idsByBand.entries())) {
      await adminSupabaseRequest(
        "PATCH",
        `/energy_rows?id=in.(${ids.join(",")})&charge_kind=eq.energy&tariff_band=is.null`,
        { tariff_band: band },
        "return=minimal"
      );
      updated += ids.length;
  ...
  const unresolved = await adminSupabaseCount(
    "/energy_rows?select=id&charge_kind=eq.energy&tariff_band=is.null&limit=1"
  );
